...
class CtxPrompt:
    ...
    def chatgpt_get_response(cls, prompt: Union[str, List[str]], examplars: List[List[Tuple[str, str]]] = [[]], max_tokens: int = 2048, api_key: str = None):
        ...
        # return generations
        responses = []
        for p, ex in zip(prompt, examplars):
            # (Optional) check token length
            assert len(p.split()) <= max_tokens, "Prompt is too long."

            # Build the messages. The old code tries something like:
            # [
            #   {'role': 'user' if i==0 else 'assistant', 'content': ex[i]},
            #   ...
            # ]
            # Then finally we add the user's actual question as a 'user' message.
            # EXACT logic depends on how your examplars are structured. 
            # Here's a simpler example:

            messages = []
            for i, (sys_str, asst_str) in enumerate(ex):
                messages.append({"role": "system", "content": sys_str})
                messages.append({"role": "assistant", "content": asst_str})

            # Then add the actual user question as the last message
            ...
            #    We assume HFmodel_call returns:
            #    { "model": "...", "choices": [ { "text": "...", ...} ] }
            if "choices" in result and len(result["choices"]) > 0:
                # We'll take the first choice
                gen_text = result["choices"][0]["text"]
            else:
                gen_text = ""

            responses.append(gen_text)

        # If single input was given, return single. Else return list.
        if is_single:
