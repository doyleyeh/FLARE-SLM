    ...
    def complete(
        ...
        if is_chat_model:
            ...
            if 'max_tokens' in params:
                params['max_tokens'] = max(1, params['s'])  # 0 is not allowed for chatgpt
...
def query_agent_worker(
    ...
    while True:
        ...
        if type(batch) is str and batch == 'DONE':
            break
        # Convert the dataset examples to CtxPrompt objects
        prompts = [CtxPrompt.from_dict(example) for example in batch]
        # Just pass api_key=None
    ...
    if not args.multiprocess:
        ...
        with tqdm(total=len(data)) as pbar, open(args.output, 'w') as fout:
            for b in range(0, len(data), args.batch_size):
                ...
                prompts = [CtxPrompt.from_dict(ex) for ex in batch]

                # Just call qagent with no key
                generations, probs, retrievals, traces = qagent.prompt(prompts, api_key=None)
                retrievals = retrievals or [None] * len(generations)
                traces = traces or [None] * len(generations)
        ...
        # MULTI-PROCESS flow, no KeyManager
        input_queue = Queue()
        output_queue = Queue()

        # Start N worker processes
        n_processes = 2  # or any number you want
        processes = []
        for _ in range(n_processes):
            p = Process(target=query_agent_worker, args=(qagent, input_queue, output_queue))
            p.daemon = True
            p.start()
            processes.append(p)
